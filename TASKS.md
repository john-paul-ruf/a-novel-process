# Revision Complete — THE FREE COLONY

## Structure Phase: Locked ✅

All major structural decisions complete. Ready for Phase 2: First Draft.

---

## Suri Park Death — **LOCKED: Ch 13 - The Channels**
✅ **COMPLETE**

**The beat**: Month 12, during channel exploration. Cole has just decoded the ancient system—learned that the channels are infrastructure designed to manage the creep during orbital cycles. He brings Suri (and others) to map the channel network deeper. A section that ran dormant through earlier exploration suddenly activates as the cycle deepens. The mineralized creep compound flows again. Suri is in a narrow cut studying the mineral deposits. The compound—the planet's own infrastructure—dissolves them.

**Responsibility**: Cole didn't miscalculate or act recklessly. The channels *were* dormant. But Cole brought Suri there. Cole was the one confident enough to explore further, confident he'd finally *understood* the system. Suri dies while Cole is staging a small victory. Cole wasn't wrong—just incomplete, and didn't know it.

**Thematic weight**: Completes the trilogy of Cole's failures: Ch 8 (protocol fails), Ch 16 (medicine fails), Ch 13 (understanding fails). By Ch 13, Cole thinks he's solved it. Suri reminds him the planet keeps changing underneath what he learns. The math keeps breaking.

**Death count**: 36 → 35 (Ch 13)

**Updated in**: story-bible.md (Suri Park character entry, continuity flags), scene-outline.md (Ch 13 detailed beats with death), timeline

---

## Summary: All Questions Locked

1. ✅ **Garrett's wound**: Failed construction co-op. Lost custody to debt. "Freedom" = geography as escape.
2. ✅ **Named deaths**: Ch 8 (unnamed, protocol failure), Ch 13 (Suri, overconfidence), Ch 26 (Joss injured), Ch 27 (Haruki, Joss complications). Death math: 47 → 44 (Ch 5) → 39 (Ch 8) → 36 (Ch 12) → 35 (Ch 13) → 32 (Ch 16) → 31 (Ch 19) → 26 (Ch 20) → 24 (Ch 22) → 22 (Ch 27).
3. ✅ **Shuttle temptation**: Garrett Polk nearly boards (debts forgiven = daughter). Chooses against himself.
4. ✅ **Colony name**: Holdfast. Vote in Ch 23, natural usage Ch 24+.
5. ✅ **Suri Park**: Ch 13, dies in channels while Cole is confident he's understood. Incomplete understanding kills.

---

---

## Continuity & Timeline Errors Found (Draft 1 QA)

### Error 1: Phantom Death in Chapter 15
**Location**: Ch 15 opening count vs. closing count  
**Status**: ✅ RESOLVED  
**Decision**: Make forest death explicit. Drop to 34.  
**Rationale**: Thematic arc — Suri's death (planetary indifference) followed by this death (human psychological fracture). Two deaths in five months shows the colony breaking at its nerve endings, not just its equipment. Explicit death = clarity; phantom death = draft error.  
**Updated**: scene-outline.md (Ch 15 beat 2, death count tracker), story-bible.md (Ch 15 timeline entry)

### Error 2: Ch 8 Timeline Table Mismatch
**Location**: story-bible.md timeline table vs. Chapter 8 text  
**Issue**: Timeline labels Ch 8 deaths as "disease, exposure, suicide" but the actual chapter describes the third death as an infection that resists antibiotics. Thematically, these are different: suicide = choice/despair vs. planetary biology defeating Earth medicine = the environment as active antagonist.  
**Action**: Update timeline table to reflect what the chapter text actually says. The chapter text is stronger; don't downgrade it to match a placeholder label.

---

## Structural Questions for Draft 1

### Question 1: Planted "Expecting Children" Beat (Ch 25)
**Planted**: Ch 25, the realization that children born on 16b aren't original signatories — legally interesting, opens a new conflict category.  
**Problem**: Introduced and dropped. Never fires as a dramatic or thematic beat.  
**Options**:  
- **A**: Give it a scene in Ch 26-27 (even brief interiority — Cole thinking about new life on land that's about to become theirs, what inheritance means, the weight of building toward a future).  
- **B**: Cut it cleanly now if it doesn't serve the third-act focus.  
**Rationale**: A planted gun the reader notices but never hears go off reads as a draft artifact. Decide now to either fire it or retire it.

### Question 2: Five Deaths in Ch 20's "Milder" Year 3
**Location**: Ch 20, Year 3, month 1-3  
**Problem**: Kill rate of 31 → 26 (five deaths in three months) during a period described as *less dangerous* than previous cycles. That's nearly identical to the fever chapter kill rate, which breaks the narrative logic of "we're past the worst of it."  
**Options**:  
- **A**: Justify the deaths harder in text (compounding fatigue, accumulated equipment failure, carelessness because crisis feels past, no redundancy left).  
- **B**: Trim to three or four deaths (31 → 27 or 28), reset expectations, adjust downstream deaths accordingly.  
**Impact**: This pacing choice shapes how much wind the colony still has in its sails going into Year 4.

### Question 3: Part 3 Temporal Compression (Ch 23, 26)
**Scope**: Part 3 covers ~30 months in 7 chapters vs. Part 1's 6 months in 9 chapters. Right instinct: survival granularity fades as drama shifts to holding the line.  
**Risk**: Ch 23 (all of Year 4) and Ch 26 (8 months) are carrying a lot of elapsed time. Draft danger: these chapters reading as montage/summary rather than scene, breaking the reader's in-body connection.  
**Plan**: Within each compressed chapter, anchor at least two or three real-time moments (specific days, conversations, tactile scenes) to keep the reader in-body rather than in-abstract. Don't let time dilation flatten the emotional texture.

---

## Next: Phase 2 First Draft
Resolve the two continuity errors and three structural questions above before drafting each affected chapter.
